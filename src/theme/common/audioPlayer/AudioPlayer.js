...
class AudioPlayer {
  ...
  wrapElement(audioElement) {
    ...
    this.player
      .querySelector(".ap-playpause-button button")
      .setAttribute("aria-controls", `ap-${audioElement.id}`);
    this.player
      .querySelector(".ap-volume-button button")
      .setAttribute("aria-controls", `ap-${audioElement.id}`);
    this.player
      .querySelector(".ap-time-total.ap-time-slider")
      .setAttribute("aria-valuemax", audioElement.dataset["length"]);
    this.player
      .querySelector(".ap-time-total.ap-time-slider")
      .setAttribute("aria-valuenow", 0);

    // Set the static duration time string after first formatting the number of seconds
    this.player.querySelector(".ap-duration").innerText = this.formatTime(
      audioElement.dataset["length"]
    );
    //
  ...
  bindEvents() {
    // Play/Pause
    this.player
      .querySelectorAll(".styled-player button[title=Play]")
      .forEach((button) => {
        button.addEventListener("click", (e) => {
          ...
          ["ap-play", "ap-pause"].map((btn) =>
            e.target.parentNode.classList.toggle(btn)
          );
        });
      });
    ...
    // Volume
    this.player
      .querySelector(".ap-horizontal-volume-slider")
    ...
    // Mute
    this.player
      .querySelector(".ap-volume-button")
      .addEventListener("click", (e) => {
        // Get the nativePlayer element that is wrapped in this custom player skin
        const audioElement = this.audioElement;
        ...
        // Toggle the svg sprite icon
        ["ap-mute", "ap-unmute"].map((div) =>
          e.target.parentNode.classList.toggle(div)
        );
      });

    // Seek
    this.player
      .querySelector(".ap-time-slider")
      .addEventListener("click", (e) => {
        const audioElement = this.audioElement;
        // Get the horizontal click position as a value between 0 and 1
        const rect = e.currentTarget.getBoundingClientRect();
    ...
    this.audioElement.addEventListener("timeupdate", () => {
      ...
      player.querySelector(
        ".ap-currenttime-container .ap-currenttime"
      ).innerText = this.formatTime(elapsed);

      // Update the buffer bar
      player.querySelector(
        ".ap-time-slider .ap-time-loaded"
      ).style.transform = `scaleX(${buffered})`;

      // Update the progress bar
      player.querySelector(
        ".ap-time-slider .ap-time-current"
      ).style.transform = `scaleX(${progress})`;

      // Update the volume bar
      player.querySelector(
        ".ap-horizontal-volume-current"
      ).style.width = volume;

      // Update aria values
      player
        .querySelector(".ap-horizontal-volume-current")
        .setAttribute("aria-valuenow", volume);
      player
        .querySelector(".ap-time-total.ap-time-slider")
        .setAttribute("aria-valuenow", parseInt(elapsed));
      player
        .querySelector(".ap-time-total.ap-time-slider")
        .setAttribute("aria-valuetext", this.formatTime(elapsed));
    });
  }
}
